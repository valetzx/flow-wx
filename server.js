...
app.get('/api/wx', async (req, res) => {
  try {
    ...
    const results = await Promise.allSettled(wxArticles.map(a => scrapeWx(a, cheerio, randomSentence)));
    const merged = {};
    results.forEach((r, i) => {
...
app.get('/api/bil', async (_req, res) => {
  try {
    ...
    const results = await Promise.allSettled(bilArticles.map(a => scrapeBil(a, cheerio, randomSentence)));
    const merged = {};
    results.forEach((r, i) => {
      if (r.status === 'fulfilled') {
        Object.assign(merged, r.value);
